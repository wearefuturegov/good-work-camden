  ...
  def show
    ...
    when "4"
      ...
      if params[:type] == "adviser"
        ...
        @coordinates = [response["result"]["latitude"], response["result"]["longitude"]]
        @services = Service.where.not(postcode: nil).joins(:tags).where("tags.tag": tags).distinct.near(@coordinates).limit(5)
